...
- **Source mockups** ("PP V2" imported Claude.ai project): a `docs/` folder containing HTML mockup files named `acode.html`...`zcode.html` and `1code.html`...`Ncode.html` (50 files as of this note: 26 letter-files + 24 numbered files, numbered `1`–`19` and `21`–`26` — note `20code.html` does not exist, that's not a gap to fill). Each new chat, re-check this folder via the imported project's file listing (`.project-cache/<id>/docs/*.html`, readable directly with Read/Glob — no bash needed) — the user periodically adds new mockup files here, and the recurring instruction is: *"Lets get building based on the new items added to the project PP V2."* That means: scan `docs/*.html`, diff against tools already built (see list below), read any new ones, and build them following the pattern below. **Some new mockup files have no `<title>` tag** (seen with `21code.html`) — if a title grep comes up empty, grep for `<h1`/`<h2 class="font-headline` instead to find the workshop name in the page body.

**Status as of this note: all 50 files in the docs folder have been consumed** — 48 built as tools, 2 skipped as duplicates of already-built tools: `13code.html` ("SCQA Strategy Workspace", duplicate of `scqa`) and `22code.html` ("First Principles Workshop" / "SpaceX-Style Cost Deconstruction", duplicate of the already-built `first_principles` tool — same assumption→challenge→fundamental-truth structure, just a different visual mockup). Files `15code.html`–`18code.html` (BLUF Workshop, Executive Decision SIR, Tell → Show → Tell Workshop, Initiative Workshop) were built in commit `6dc1a43`. Files `19code.html`, `21code.html`, `23code.html`–`26code.html` (Cash Flow Projection, Empathy Map Workshop, JTBD Workshop, P&L Forecasting, Customer Journey Workspace, User Persona Workshop) were the latest new batch, built in commit `2831922`. If the user says the recurring build phrase again, the first step is re-listing `docs/*.html` to check whether new files have been added since (currently up to `26code.html`).

Note on near-duplicates that were NOT skipped: `21code.html` ("Solopreneur Persona Workshop", actually an Empathy Map by structure — Says/Does/Thinks/Feels + Pain/Gain) and `26code.html` ("User Persona Workshop", a persona profile card — demographics/motivations/day-in-life/quote) both have "persona" in the name but are structurally distinct frameworks, so both were built as separate tools (`empathy_map` and `user_persona`). Similarly `19code.html` ("Cash Flow Projection", a month-by-month ledger + sensitivity sliders) was judged distinct enough from the existing `burn_rate` tool (category-based fixed/variable costs → runway) to build separately, as was `24code.html` ("P&L Forecasting", revenue/expense lines → scenario-based 12-month EBITDA) against both `burn_rate` and the new `cash_flow_projection`. When in doubt on a near-duplicate, compare the actual field-level data shape, not just the title — check the existing tool's `analyze*()` function and state shape before deciding to skip or build.
...
## Tools built so far (52 framework catalog entries, ids 1–52)
...
- Latest commit on `main`: `2831922` ("Add Cash Flow Projection, Empathy Map, JTBD, P&L Forecasting, Customer Journey, and User Persona workshops").
